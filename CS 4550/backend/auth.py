...
from backend.entities import User, UserFound
from backend.database import EntityDuplicateException
...
@auth_router.post("/registration", response_model=UserFound, status_code=201)
def register_new_user(
    ...
    for u in Users:
        if u.username == user.username:
            raise EntityDuplicateException(entity_name="User", entity_field='username', entity_value=registration.username)
        if u.email == user.email:
            raise EntityDuplicateException(entity_name="User", entity_field='email', entity_value=registration.email)
